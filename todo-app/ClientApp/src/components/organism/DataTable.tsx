...
export const DataTable: FC<Props> = memo((props: Props) => {
  ...
  const onClickTableRow = useCallback((id: number) => {
    // ダイアログの表示要素を切り替える
    setUpdateFlag(true);
    // 選択したTodoを設定する
        ...
        <Tbody>
          {
            !toDos ? <></> : toDos.map((toDo) => (
              (date > toDo.expectedEndOfDate! ?
                  <Tr key={toDo.id} onClick={ () => onClickTableRow(toDo.id as number)} bg="red.100" >
                    <Td key={toDo.id! + 5}>{toDo.id}</Td>
                    <Td key={toDo.id! + 6}>{controllStringlength(toDo.contents)}</Td>
                   <Td key={toDo.id! + 7}>{toDo.expectedEndOfDate}</Td>
                   <Td key={toDo.id! + 8}>{toDo.endOfDate}</Td>
                  </Tr>
              : <Tr key={toDo.id} onClick={ () => onClickTableRow(toDo.id as number)} >
                  <Td key={toDo.id! + 5}>{toDo.id}</Td>
                  <Td key={toDo.id! + 6}>{controllStringlength(toDo.contents)}</Td>
                  <Td key={toDo.id! + 7}>{toDo.expectedEndOfDate}</Td>
                  <Td key={toDo.id! + 8}>{toDo.endOfDate}</Td>
